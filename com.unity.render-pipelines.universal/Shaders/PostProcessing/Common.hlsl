...
half3 FXAAFetch(float2 coords, float2 offset, TEXTURE2D_PARAM(InputTexture, InputSampler))
{
    float2 uv = coords + offset;
    return SAMPLE_TEXTURE2D_X(InputTexture, InputSampler, uv).xyz;
}

half3 FXAALoad(int2 icoords, int idx, int idy, float4 sourceSize, TEXTURE2D_PARAM(InputTexture, InputSampler))
{
    #if SHADER_API_GLES
...
half3 ApplyFXAA(half3 color, float2 positionNDC, int2 positionSS, float4 sourceSize, TEXTURE2D_PARAM(InputTexture, InputSampler))
{
    // Edge detection
    half3 rgbNW = FXAALoad(positionSS, -1, -1, sourceSize, TEXTURE2D_ARGS(InputTexture, InputSampler));
    half3 rgbNE = FXAALoad(positionSS,  1, -1, sourceSize, TEXTURE2D_ARGS(InputTexture, InputSampler));
    half3 rgbSW = FXAALoad(positionSS, -1,  1, sourceSize, TEXTURE2D_ARGS(InputTexture, InputSampler));
    half3 rgbSE = FXAALoad(positionSS,  1,  1, sourceSize, TEXTURE2D_ARGS(InputTexture, InputSampler));
    ...
    // Blur
    half3 rgb03 = FXAAFetch(positionNDC, dir * (0.0 / 3.0 - 0.5), TEXTURE2D_ARGS(InputTexture, InputSampler));
    half3 rgb13 = FXAAFetch(positionNDC, dir * (1.0 / 3.0 - 0.5), TEXTURE2D_ARGS(InputTexture, InputSampler));
    half3 rgb23 = FXAAFetch(positionNDC, dir * (2.0 / 3.0 - 0.5), TEXTURE2D_ARGS(InputTexture, InputSampler));
    half3 rgb33 = FXAAFetch(positionNDC, dir * (3.0 / 3.0 - 0.5), TEXTURE2D_ARGS(InputTexture, InputSampler));
